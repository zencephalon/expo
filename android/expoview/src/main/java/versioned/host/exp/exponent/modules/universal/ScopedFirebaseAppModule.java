...
import expo.modules.firebase.app.FirebaseAppModule;
import expo.modules.firebase.app.FirebaseAppOptions;
...
public class ScopedFirebaseAppModule extends FirebaseAppModule {
  ...
  public ScopedFirebaseAppModule(Context context, JSONObject manifest, ExperienceId experienceId) {
    super(context);
  ...
  private static FirebaseOptions getOptionsFromManifest(JSONObject manifest) {
    try {
      ...
      if (!json.containsKey("appId"))
        return null;
      return FirebaseAppOptions.fromJSON(json);
    } catch (Exception err) {
      return null;
    }
  }
