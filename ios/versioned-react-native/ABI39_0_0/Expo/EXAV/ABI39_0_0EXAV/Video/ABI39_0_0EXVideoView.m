...
- (void)_removePlayer
{
  if (_requestedFullscreenChangeRejecter) {
    ...
    _requestedFullscreenChangeRejecter(@"E_VIDEO_FULLSCREEN", errorMessage, ABI39_0_0UMErrorWithMessage(errorMessage));
    _requestedFullscreenChangeResolver = nil;
    _requestedFullscreenChangeRejecter = nil;
    _requestedFullscreenChange = NO;
  }
  ...
  _data = [[ABI39_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                           ...
                           withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus, NSString *error) {
                             ...
                             if (success) {
                               ...
                               [self _removePlayer];
                               if (reject) {
                                 reject(@"E_VIDEO_NOTCREATED", error, ABI39_0_0UMErrorWithMessage(error));
                               }
                               [self _callErrorCallback:error];
                             }
                           }];
             ...
             rejecter:(ABI39_0_0UMPromiseRejectBlock)reject
{
  if (!_data) {
    ...
    if (reject) {
      ...
      reject(@"E_VIDEO_FULLSCREEN", errorMessage, ABI39_0_0UMErrorWithMessage(errorMessage));
    }
    return;
