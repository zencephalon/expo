...
#import "ABI39_0_0EXUpdates.h"
#import <ABI39_0_0React/ABI39_0_0RCTUIManager.h>
#import <ABI39_0_0React/ABI39_0_0RCTBridge.h>

NSString * const ABI39_0_0EXUpdatesEventName = @"Exponent.nativeUpdatesEvent";
NSString * const ABI39_0_0EXUpdatesErrorEventType = @"error";
NSString * const ABI39_0_0EXUpdatesNotAvailableEventType = @"noUpdateAvailable";
NSString * const ABI39_0_0EXUpdatesDownloadStartEventType = @"downloadStart";
NSString * const ABI39_0_0EXUpdatesDownloadProgressEventType = @"downloadProgress";
NSString * const ABI39_0_0EXUpdatesDownloadFinishedEventType = @"downloadFinished";

ABI39_0_0EX_DEFINE_SCOPED_MODULE_GETTER(ABI39_0_0EXUpdates, updates)

@interface ABI39_0_0EXUpdates ()
...
@implementation ABI39_0_0EXUpdates

@synthesize bridge = _bridge;

ABI39_0_0EX_EXPORT_SCOPED_MODULE(ExponentUpdates, UpdatesManager)
...
{
  [_bridge enqueueJSCall:@"ABI39_0_0RCTDeviceEventEmitter.emit" args:@[ABI39_0_0EXUpdatesEventName, body]];
}
...
ABI39_0_0RCT_EXPORT_METHOD(reloadFromCache)
{
  [_kernelUpdatesServiceDelegate updatesModuleDidSelectReloadFromCache:self];
}

ABI39_0_0RCT_EXPORT_METHOD(checkForUpdateAsync:(ABI39_0_0RCTPromiseResolveBlock)resolve
                             rejecter:(ABI39_0_0RCTPromiseRejectBlock)reject)
{
  ...
  [_kernelUpdatesServiceDelegate updatesModule:self didRequestManifestWithCacheBehavior:ABI39_0_0EXManifestNoCache success:^(NSDictionary * _Nonnull manifest) {
...
ABI39_0_0RCT_EXPORT_METHOD(fetchUpdateAsync:(ABI39_0_0RCTPromiseResolveBlock)resolve
                          rejecter:(ABI39_0_0RCTPromiseRejectBlock)reject)
{
  if ([self _areDevToolsEnabledWithManifest:_manifest]) {
    [self sendEventWithBody:@{
                               @"type": ABI39_0_0EXUpdatesErrorEventType,
                               @"message": @"Cannot fetch updates in dev mode"
                               }];
    reject(@"E_FETCH_UPDATE_FAILED", @"Cannot fetch updates in dev mode", nil);
    return;
  }
  [_kernelUpdatesServiceDelegate updatesModule:self didRequestManifestWithCacheBehavior:ABI39_0_0EXManifestPrepareToCache success:^(NSDictionary * _Nonnull manifest) {
    NSString *currentRevisionId = self->_manifest[@"revisionId"];
    NSString *newRevisionId = manifest[@"revisionId"];
    if (currentRevisionId && newRevisionId && [currentRevisionId isEqualToString:newRevisionId]) {
      [self sendEventWithBody:@{ @"type": ABI39_0_0EXUpdatesNotAvailableEventType }];
      resolve(nil);
      return;
    }
    ...
    void (^progressBlock)(NSDictionary * _Nonnull) = ^void(NSDictionary * _Nonnull progressDict) {
      ...
      eventBody[@"type"] = ABI39_0_0EXUpdatesDownloadProgressEventType;
      [self sendEventWithBody:eventBody];
    };
    void (^successBlock)(NSData * _Nonnull) = ^void(NSData * _Nonnull data) {
      [self sendEventWithBody:@{
                                 @"type": ABI39_0_0EXUpdatesDownloadFinishedEventType,
                                 @"manifest": manifest
                                 }];
      resolve(manifest);
    };
    ...
    [self sendEventWithBody:@{ @"type": ABI39_0_0EXUpdatesDownloadStartEventType }];
