...
public class EmbeddedBareAppLoader {

  private static final String TAG = EmbeddedBareAppLoader.class.getSimpleName();
  ...
  public EmbeddedBareAppLoader(Context context, UpdatesDatabase database, File updatesDirectory) {
    mContext = context;
    mDatabase = database;
  ...
  public boolean loadBareUpdate(Manifest manifest) {
    boolean success = processManifest(manifest);
    reset();
    return success;
  }
  ...
  private void copyAllAssets(ArrayList<AssetEntity> assetList) {
    ...
    // TODO: if no skipped/errored assets, also copy the bundle
    // and then mark update as ready
  }
